...
public class C11ThrowException {
    ...
    // jvm 으로 익셉션 처리를 위임. -> jvm 은 오류메세지 출력 -> 기본 동작
    // main 에서는 try ~ catch 로 프로그램 내에서 처리되도록 한다.

    System.out.println("프로그램 종료합니다.");
        
    }

    // 메소드의 throws 하는 예외는 RuntimeException (언체크 익셉션) 은 적용이 안된다.
    // checkAge 사용하는쪽에서 IllegalAge 처리해줘
    public void checkAge(int age) throws IllegalAgeException{     // 체크 익셉션 메소드로 정의
        ...
        if(age<20){
            ...
            throw new IllegalAgeException("잘못된 나이 입력 값 입니다.");
        }else {
