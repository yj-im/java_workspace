...
public class C17FileReaderTest {
    ...
    public static void readUTF(){
        ...
        Scanner sc=null;
        String filename="입출력.md";    // UTF-8 로 인코딩된 파일
        try{
    ...
    public static void readByLine(){
        ...
        try {
            fr=new FileReader("입출력.md");
            sc=new Scanner(fr); // 파일을 읽기 위해서 Reader 타입 클래스를 인자로 한다.
            // hasXXX 은 iterator(반복자) 를 통해서 다음 내용이 있을 때 동작하는 방식.
            // iterator 인터페이스 구현 클래스 : hasNext() 메소드, nextXXX() 메소드가 있음.
            while(sc.hasNext()){    // 읽어올 다음줄이 있느냐?, 참이면 
                // System.out.println((char)ch);
                // sb.append((char)ch);    // 읽은 문자 한개씩을 문자열로 만들기 위해 추가
                line=sc.nextLine();     // 위에 참이면, 다음줄 읽어오기
