...
// Math.random() 메소드 활용
public class B13RandonTest {

    // 실행에 필요한 데이터 : 값을 직접 대입 / 표준 입력(키보드) / 다량의 임의 데이터를 만들기.
    public static void main(String[] args) {
        
        System.out.println("임의의 double 값 5개를 만들기 : 범위 0 ~ 1 사이");
        ...
        // start ~ end 사이의 정수값을 랜덤 생성 한다면
        // (int) (Math.random()*(end-start+1)) + start
        // start=1, end=45
        System.out.println("로또 추첨번호 뽑기");
        int start=1, end=45;
        for(int i=0; i<6; i++)
        System.out.println((int)(Math.random()*(end-start+1))+start);

        System.out.println("두자리 정수 랜덤 만들기");
        // 수학 학습지 문제의 숫자를 랜덤하게 만들기 : 두자리 덧셈과 뺄셈. 11~99
        start=11; end=99;
        for(int i=0; i<6; i++)
        System.out.println((int)(Math.random()*(end-start+1))+start);
    }
