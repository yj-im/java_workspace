...
public class B17ProductCartApp {
    public static void main(String[] args) {
        ...
        cart.add(new Product("크리넥스티슈", 15600, "크리넥스"));   // 인덱스 3번

        System.out.println("cart 리스트의 요소 개수 : "+cart.size());   // 배열이름 .length 와 같음
        ...
        // cart 리스트에 담긴 Product 객체의 물품 가격 합계를  구해보세요.

        int sum=0;
        // 1) 인덱스 사용 for문 : cart.get(i)
        ...
        sum=0;
        System.out.println("//////////////////////");
        // 2) 향상 for문 : cart.get(i) 는 자동으로 처리(Product 타입 변수에 하나씩 참조)됨.
        for(Product p : cart){
            sum+=p.getPrice();
        }
        System.out.println("cart 가격 합계 : "+sum);
...
class Product{
    ...
    @Override       // 상속에서 배웁니다.  @Override 기호 없어도 실행은 된다.
    public String toString() {
        return "[상품명=" + proName + ", 가격=" + price + ", 제조사=" + company + "]";
    }

    // '문자열을 리턴'하는 toString() 메소드를 정의하면 객체 변수 출력은 참조값이 아니라
    //                     toString() 메소드 리턴받은 문자열을 출력함. 



    // getter, setter 는 필요시 만들어 본다.
    public int getPrice(){
        return this.price;
    }
}
