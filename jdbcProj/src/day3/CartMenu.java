...
public class CartMenu {
  ...
  public void start() {
    ...
    //   1. 아이디 존재 여부 확인 필요. 아이디 존재할때만 아래 실행하기 -> 각자 해보셈
  ...
  public void showCartList(){
    int totlaprice=0;    // 결제금액 초기화
    System.out.println("----- 장바구니 목록 -----");
    // 상품코드, 수량
    for(int i =0;i<list.size();i++){
      BuyVo temp = list.get(i);
      System.out.println(i + "\t" + temp.getPcode() + "\t" + temp.getQuantity());
      totlaprice+=(temp.getQuantity()*priceTables.get(temp.getPcode()));
    }
    // 장바구니 상품들 총 결제금액을 구할 예정
    System.out.println("총 결제 금액 : "+totlaprice);
  }
